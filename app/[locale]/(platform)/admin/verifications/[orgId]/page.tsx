...
export default async function Page({
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8 md:p-8">
      <div className="mb-2">
        <Link href="/admin/verifications" className="text-caption text-[var(--ink-3)] hover:text-[var(--ink)]">
          {t('back')}
        </Link>
      </div>
      <div className="flex items-start justify-between gap-4 mb-8 flex-wrap">
        <div>
          <h1 className="text-display font-[family-name:var(--font-display)] mb-2 inline-flex items-center gap-3 flex-wrap">
            {organization.name}
            <StatusPill tone={toneForVerificationStatus(status)}>{tStatus(status)}</StatusPill>
          </h1>
          {organization.description && (
            <p className="text-body text-[var(--ink-2)] max-w-prose">{organization.description}</p>
          )}
        </div>
      </div>
          ...
          <div className="border border-[var(--border-color)] rounded-md p-4 bg-[var(--surface)] space-y-2 text-body">
            ...
            {organization.website && (
              <div className="flex justify-between gap-3">
                ...
                <a href={organization.website} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-[var(--brand-600)] hover:text-[var(--brand-700)] break-all">
                  {organization.website}
                  <ExternalLink size={13} strokeWidth={2} aria-hidden className="shrink-0" />
                </a>
              </div>
            )}
            <div className="flex justify-between gap-3">
      ...
      <section className="mb-8">
        ...
        {organization.rneUrl ? (
          isPdf ? (
            <div>
              ...
              <a
                ...
                className="inline-block mt-2 text-caption text-[var(--brand-700)] hover:underline"
              >
                {t('openOriginal')}
              </a>
            </div>
          ...
          <div className="border border-dashed border-[var(--border-color)] rounded-md p-6 text-center text-[var(--ink-3)] text-caption">
            {t('noRne')}
          </div>
        )}
      </section>
