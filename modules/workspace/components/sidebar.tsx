  ...
  return (
    <aside className="ws-side">
      ...
      <div className="ws-side-section">
        ...
        {data.role === 'intern'
          ? data.activeWorkspaces.map((w, i) => (
              <div
                key={w.id}
                className={`ws-side-sub ${i === 0 ? 'active' : ''} ${w.live ? 'live' : 'done'}`}
              >
                <span className="dot" />
          ...
          : data.activeProjects.map((p, projectIndex) => (
              <div key={p.id} style={projectIndex > 0 ? { marginTop: 12 } : undefined}>
                ...
                {p.status === 'active' && projectIndex === 0 && (
                  <a className="ws-side-projhub">→ Project hub</a>
                )}
                {p.workspaces.map((w, wi) => (
                  <div
                    key={w.id}
                    className={`ws-side-sub in-proj ${wi === p.workspaces.length - 1 ? 'last' : ''} ${w.live ? 'live' : 'done'} ${projectIndex === 0 && wi === 0 ? 'active' : ''}`}
                  >
                    <span className="dot" />
                    <span className="truncate">{w.label}</span>
                  </div>
