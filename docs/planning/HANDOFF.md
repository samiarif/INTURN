...
**Atelier Phase 1 EXECUTED (2026-06-10, later) — branch `feat/atelier-tokens`, stacked on `fix/audit-quick-wins` (5 commits, awaiting review with its base).** Plan: `docs/superpowers/plans/2026-06-10-atelier-phase-1-tokens.md`. What landed: (1) shadcn semantics rebound — `--primary` = ink (inverts in dark via ink/surface flip), `--ring` = brand-500 (brand-300 in dark), `--input` = slate `--border-color`; (2) Button `brand` variant + THE RULE (violet only at value moments) applied to ~20 visible chrome buttons (error pages, marketplace search, dashboard/saved/applications/records/community CTAs, sprint save/create/edit, admin resolve → ink; apply/publish → `variant="brand"`; AI accepts + view-record stay violet); ~44 patch sites remain for per-screen passes — sweep under THE RULE in the plan; (3) one radius scale 4/6/8/12 (fixed steps replace the calc chain; dead `--radius-ws-*` deleted; `.ws` radius fork still deferred); (4) one elevation system (`--elev-*` bridged as `shadow-card/-hover/-pop` utilities; colliding `--shadow-xs..lg` deleted; `.ph-brief/.db-card/.db-stat/.pi-card` get resting lift); (5) `.ui-rise-col` staggered entrance wired on the 5 key screens. Gate: 629 tests / typecheck / lint / i18n / prod build all green. After-screenshots verified live (marketplace search = ink, dashboard's only violet = the profile CTA).

**Website-as-front-door SHIPPED (2026-06-10, evening) — the marketing homepage now IS the platform landing on :3000** (`ab70d96` on `feat/atelier-tokens`): ported from `../inturn-web` with 186 localized strings (FR vous-form, the hero joke adapted: « De vrais stages. ~~Des cafés à servir.~~ »), CTAs wired into the product, `app/landing.css` fully scoped under `.mk-root` (the site's tokens reuse platform token NAMES with different values — they must never reach `:root`). Unported page links = section anchors or coming-soon spans. **UPDATE same evening: ALL six remaining pages ported** — (site) route group with shared layout; how-it-works, for-companies, for-interns, for-universities, virtual-internships, and **verify wired to the real /records/[token] lookup**; nav uses real routes + usePathname active state; +429 locale keys (2,621 aligned); 6 new public-route patterns tested. The standalone ../inturn-web app is now fully superseded (kept as source archive). Remaining site work: a contact page (university CTAs are coming-soon spans), Arabic teaser in the footer, and pruning unused mk-* CSS for unported-page leftovers. Also that session: light rail restored (Sam's call — ink-rail experiment reverted, mono role eyebrow kept), marketplace search fixed (city/location searchable + EN form locale loss; FR-vocabulary gap documented as a decision), per-visit confetti bug fixed, entrance animations rebuilt fail-open via @starting-style.

**Atelier SCREEN PASSES complete (2026-06-10, night)** — the audit's entire "unstyled tier" got the treatment, one verified commit each: **team** (`ea61a6b`), **university ×3 surfaces** (`f4bca09`), **admin ×6 pages incl. the last raw-hex buttons in the codebase** (`b12e597`), **hiring flow** (inbox/detail/compare, `54cc302` — also fixed two dark-mode invisible-text bugs), **form bodies** (apply/onboarding/account-edit/both creation monoliths, `5eb96d5` — surgical className-only edits). Idioms now platform-wide: mono brand-700 eyebrows + counts, --elev-card resting shadows, StatusPill everywhere (per-page color hashes deleted), shared gradient Avatar, KPI stat tiles, ink chrome with violet strictly at value moments (apply/publish/accept/approve/submit-livrable). Each pass browser-verified light+dark. **Remaining design debt (documented, not blocking):** the ~1,954 arbitrary-var wrapper codemod, px font-sizes → --text-* in the CSS families, dissolving .ph-/.db-/.ex-/.pi-/.rec- into primitives, the .ws radius fork, ~40 leftover violet patches on minor screens, mk-* CSS pruning, a contact page, the FR-search language decision.
...
- **Branch/state:** all work is on **`main`** and pushed to **`origin/main`**. **344 tests pass** / 2 opt-in integration tests skipped; typecheck + lint clean. `main` carried 15 previously-unpushed **Team-management** commits; this session added **four feature commits** (`2d1ac37` build fix → `4c22c73` close-the-loop) plus handoff docs, then pushed everything and **deployed to production** (commit `205ca4e` is live — see Deploy below).
...
- **Deploy is MANUAL, not git-triggered.** The Vercel project (`prj_nBj8qrunM5YlebJ19uIRbBJ8d2mx`) is **not** connected to the GitHub repo — pushing to `main` runs CI (`ci.yml`: typecheck + lint + test + build) but does **not** deploy. Production deploys are run by hand from the linked CLI: **`vercel --prod`** (builds in Vercel's cloud, runs the `prebuild` migrate against the prod DB, then aliases **`https://inturn.vercel.app`**). A failed build is non-destructive — the prior Ready deploy stays live until a new build succeeds. This session deployed **`205ca4e`** → verified live via `https://inturn.vercel.app/api/health` (`{status:ok, commit:205ca4e, env:production}`). Env to keep present in Vercel prod: `ANTHROPIC_API_KEY` (D12 assists reuse it — they degrade gracefully if absent).
