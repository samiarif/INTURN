...
> **STATUS (2026-05-30): SHIPPED — but reframed.** E24/E25/E27/E30 were built and merged to `main`, **reframed from the anonymized-aggregates dashboard described below to PER-STUDENT academic supervision**: the coordinator (*encadrant académique*) invites specific students, sees each one's internship phase via a firewalled snapshot, and reviews their versioned *rapport académique* (approve / request-revision / comment). E27's "all anonymized — aggregates only" and E30's "cannot see individual student profiles" were **superseded** by this model. The E24 schema landed as `organizations.kind` + an `organization_members` `student` role (NOT a separate `universities` table or a `profiles.university_id` FK). Still deferred: **E26** (CSV bulk invite), **E28** (convention de stage PDF), **E29** (`/for-universities` landing). As-built design + plans: `docs/superpowers/{specs,plans}/2026-05-30-university-*`; current state in `docs/planning/HANDOFF.md`. The original Week-7–8 spec is preserved below for reference.
